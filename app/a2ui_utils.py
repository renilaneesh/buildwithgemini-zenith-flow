...
_VIDEO_NOTE = "Video generated — open the Artifacts panel to view it."


def _sanitize_media_components(messages: list[dict]) -> None:
    """Replace un-fetchable <Image> or <Video> components with a short <Text> note, in place.

    Keeps the component's `id` so any parent child-reference still resolves; only
    the component body changes from Image/Video to Text. A component is kept only if its
    url is a literal http(s) link.
    """
    for m in messages:
        ...
        for c in surface.get("components") or []:
            ...
            if "Image" in comp:
                ...
                literal = url.get("literalString") if isinstance(url, dict) else None
                if isinstance(literal, str) and _HTTP_URL_RE.match(literal):
                    continue  # real link -> leave the Video alone
                c["component"] = {
                    "Text": {
                        "text": {"literalString": _VIDEO_NOTE},
                        "usageHint": "body",
                    }
                }
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        # Turn un-fetchable <Image> and <Video> URLs into a text note.
        _sanitize_media_components(messages)
